    ...
    for N, K in shot_way_pairs:
        ...
        for repeat in range(num_repeat):
            ...
            for episode in range(args.epochs):
                ...
                _loss.backward()
                optimizer.step()
                meta_train_acc.append(_acc.item())
                if (episode > 0 and episode % 100 == 0) or (episode == args.epochs-1):    
                    ...
                    for idx in range(meta_valid_num):
                        ...
                        model.eval()
                        loss_val, acc_val = loop(class_selected, id_support, id_query, n_way, k_shot, train=False)
                        meta_val_loss.append(loss_val)
            ...
            for repeat in range(num_repeat):
                ...
                results[dataset]['{}-way {}-shot {}-repeat'.format(N,K,repeat)]= meta_test_loss_total[repeat]
