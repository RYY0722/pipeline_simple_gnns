...
def train(class_selected, id_support, id_query, n_way, k_shot, args):
    model.train()
    ...
    for i in range(args.task_num):
        ...
        grad = torch.autograd.grad(loss_train, model.parameters(), create_graph=True)
        fast_weights = OrderedDict((name, param - args.lr * grad) for ((name, param), grad) in zip(fast_weights.items(), grad))

        task_generator(id_by_class, class_list_train, n_way, k_shot, n_query)
        output = model.forward_pesudo(features, adj, fast_weights)[id_query]
        labels_new = torch.LongTensor([class_selected.index(i) for i in labels[id_query]])
        ...
        for repeat in range(num_repeat):
            ...
            for episode in range(args.epochs):
                id_support, id_query, class_selected = \
                    task_generator(id_by_class, class_list_train, n_way, k_shot, n_query)
                acc_train, f1_train = train(class_selected, id_support, id_query, n_way, k_shot, args)
